...
class Model_old(nn.Module):
    ...
    def __init__(self,
        ...
        self.weighted_mean = torch.nn.Conv1d(in_channels=input_time_frame, out_channels=1, kernel_size=1)
    ...
    def gradient_reversal(self, encoding, reverse_layer_lambda):
        '''
            generate an output with the same floating values but during backpass their gradient will be negated
        '''
        reversed_feature = ReverseLayerF.apply(encoding,
                                               reverse_layer_lambda)  # ref: https://github.com/SongweiGe/scDGN/blob/master/utils/model_util.py
        return reversed_feature

    def forward(self, x, reverse_layer_lambda=0):
        ...
        x = x.permute(0, 2, 1, 3).contiguous()  # prepare the input for the Time-Extrapolator-CNN (NCTV->NTCV)

        x_domain = x.view(b, t, c * v)
        # print('3>>', x.shape)
        x_domain = self.gradient_reversal(x_domain, reverse_layer_lambda=reverse_layer_lambda)
        x_domain = self.weighted_mean(x_domain).view(b, -1)
        # print('4>>', x_domain.shape)
        x_domain = self.domain_classifier(x_domain)
...
class Model_T(nn.Module):
    ...
    def __init__(self,
        ...
        self.transformer_encoder = PoseTransformer(num_frame=input_time_frame, num_joints=joints_to_consider,
                                                   in_chans=input_channels, embed_dim_ratio=32 // self.reduce_ratio,
                                                   depth=4, num_heads=8 // self.reduce_ratio, mlp_ratio=2.,
                                                   qkv_bias=True,
                                                   qk_scale=None, drop_rate=0., attn_drop_rate=0.1, drop_path_rate=0.2,
                                                   norm_layer=None)
    ...
    def forward(self, x):
        # print('1>>', x.shape)
        x, x_domain = self.transformer_encoder(x)  # N, C, T, V -> n, t, v, c
        # print('2>>', x.shape)

        x = x.permute(0, 1, 3, 2)  # prepare the input for the Time-Extrapolator-CNN (n, t, v, c->NTCV)
    ...
    #     print('1>>', x.shape)
    #     x = self.transformer_encoder(x) # N, C, T, V -> n, t, v, c
    #     print('2>>', x.shape)
    #
    #     return x
