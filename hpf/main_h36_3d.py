...
def train():
    #model.load_state_dict(torch.load('model_adv_learn7.pth'))
    ...
    reverse_layer_lambda=0

    for epoch in range(args.n_epochs):
        print('epoch',epoch)
        print('lambda',reverse_layer_lambda)
        if epoch%2==0 and reverse_layer_lambda<=2:
          reverse_layer_lambda+=0.004

        running_loss = 0
        n = 0
        model.train()
        for cnt, (batch, current_subject) in enumerate(data_loader):
            batch = batch.to(device)
            if use_domain_adv_loss:
                current_subject = current_subject.to(device)
            ...
            sequences_predict, predicted_subject = model(sequences_train,reverse_layer_lambda)
            sequences_predict = sequences_predict.permute(0, 1, 3, 2)

            loss = mpjpe_error(sequences_predict, sequences_gt)

            if use_domain_adv_loss:
                # print('>>predicted_subject, current_subject', predicted_subject.shape, current_subject.shape)
                b = predicted_subject.shape[0]
                loss = loss + nn.functional.cross_entropy(predicted_subject.view(b, 5), current_subject.view(b))

            subject_preds += np.argmax(predicted_subject.cpu().detach().numpy(), axis=1).reshape([b]).tolist()
            subject_labels += current_subject.cpu().detach().numpy().reshape([b]).tolist()

            if cnt % 200 == 0:
                print('[%d, %5d]  training loss: %.3f' % (epoch + 1, cnt + 1, loss.item()))
                print('subject_preds[-10:], subject_labels[-10:]:',
                      subject_preds[-10:], subject_labels[-10:])
        ...
        print(f'[{epoch + 1}]: Subject classification accuracy:',
              accuracy_score(y_true=subject_labels,
                             y_pred=subject_preds))
        torch.save(model.state_dict(),
                   'checkpoints/CKPT_3D_H36M/h36_3d_25frames_ckpt')  # 'model_adv_learn2.pth') #os.path.join(args.model_path,model_name)))
        model.eval()
        with torch.no_grad():
            running_loss = 0
            n = 0
            for cnt, (batch, _) in enumerate(vald_loader):
                batch = batch.to(device)
                batch_dim = batch.shape[0]
                n += batch_dim
                ...
                sequences_predict, _ = model(sequences_train,reverse_layer_lambda)
                sequences_predict = sequences_predict.permute(0, 1, 3, 2)
            ...
            current_loss = val_loss[-1].numpy().sum()  # loss.detach().cpu().numpy().sum()
            print(
                '[Epoch: %d]  validation loss >> Current: %.3f | Best: %.3f' % (epoch + 1, current_loss, best_val_loss))
            if current_loss < best_val_loss:
                best_val_loss = current_loss
                print('----saving model-----')
                torch.save(model.state_dict(), 'model_adv_learn_best_model.pth')  # os.path.join(args.model_path,model_name))
...
def test():
    print('args.output_n:', args.output_n)
    model.load_state_dict(torch.load('model_adv_learn_best_model.pth'))  # os.path.join(args.model_path,model_name)))
    ...
    for action in actions:
        ...
        test_loader = DataLoader(dataset_test, batch_size=args.batch_size_test, shuffle=False, num_workers=0,
                                 pin_memory=True)
        for cnt, (batch, _) in enumerate(test_loader):
            with torch.no_grad():
                batch = batch.to(device)
                batch_dim = batch.shape[0]
                n += batch_dim
                ...
                sequences_predict = model(sequences_train)[0].permute(0, 1, 3, 2).contiguous().view(-1, args.output_n,
                                                                                                    len(dim_used))
