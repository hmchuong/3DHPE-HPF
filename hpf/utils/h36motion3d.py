...
class Datasets(Dataset):
    ...
    def __init__(self, data_dir,input_n,output_n,skip_rate, actions=None, split=0):
        ...
        self.p3d = {}
        self.subjects = {}
        self.subject_map = {1:0, 6:1, 7:2, 8:3, 9:4,
                            11:0, 5:0}
        self.data_idx = []
            ...
            for action_idx in np.arange(len(acts)):
                ...
                if self.split <= 1:
                    for subact in [1, 2]:  # subactions
                        if DEBUG: print("Reading subject {0}, action {1}, subaction {2}".format(subj, action, subact))
                    ...
                    if DEBUG: print("Reading subject {0}, action {1}, subaction {2}".format(subj, action, 1))
                    ...
                    if DEBUG: print("Reading subject {0}, action {1}, subaction {2}".format(subj, action, 2))
    ...
    def __getitem__(self, item):
        ...
        # current_subject = torch.zeros([5])
        # current_subject[self.subject_map[self.subjects[key]]] = 1.
        current_subject = torch.Tensor([self.subject_map[self.subjects[key]]]).long()
        # print('??')
        return self.p3d[key][fs], current_subject
