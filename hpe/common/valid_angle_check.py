...
def h36m_valid_angle_check_torch(p3d):
    ...
    # Spine2LHip
    p1 = data_all[:, 7] - data_all[:, 0] # Spine - Hip
    p2 = data_all[:, 4] - data_all[:, 0] # LHip - Hip
    cos_gt_l = cos_func(p1, p2)
    
    # Spine2RHip
    p1 = data_all[:, 7] - data_all[:, 0] # Spine - Hip
    p2 = data_all[:, 1] - data_all[:, 0] # RHip - Hip
    cos_gt_r = cos_func(p1, p2)
    valid_cos['Spine2Hip'] = torch.vstack((cos_gt_l, cos_gt_r))

    # LLeg2LThighPlane
    p0 = data_all[:, 6] - data_all[:, 5] # Leg: LFoot - LKnee
    p1 = data_all[:, 4] - data_all[:, 0] # LHip - Hip
    p2 = data_all[:, 4] - data_all[:, 5] # LHip - LKnee
    n0 = torch.cross(p1, p2, dim=1)
    cos_gt_l = cos_func(n0, p1)
    
    # RLeg2RThighPlane
    p0 = data_all[:, 3] - data_all[:, 2] # RFoot - RKnee
    p1 = data_all[:, 1] - data_all[:, 0] # RHip - Hip
    p2 = data_all[:, 1] - data_all[:, 2] # RHip - RKnee
    n0 = torch.cross(p1, p0)
    cos_gt_r = cos_func(n0, p2)
    valid_cos['Leg2HipPlane'] = torch.vstack((cos_gt_l, cos_gt_r))

    # Shoulder2Shoulder
    p0 = data_all[:, 11] - data_all[:, 8] # LSHoulder - Thorax
    p1 = data_all[:, 14] - data_all[:, 8] # RSHoulder - Thorax
    cos_gt = cos_func(p0, p1)
    valid_cos['Shoulder2Shoulder'] = cos_gt

    # Neck2Spine
    p0 = data_all[:, 9] - data_all[:, 8] # Neck - Thorax
    p1 = data_all[:, 7] - data_all[:, 0] # Spine - Hip
    cos_gt = cos_func(p0, p1)
    valid_cos['Neck2Spine'] = cos_gt

    # Shoulder2Neck
    p1 = data_all[:, 11] - data_all[:, 8] # LSHoulder - Thorax
    p2 = data_all[:, 9] - data_all[:, 8] # Neck - Thorax
    cos_gt_l = cos_func(p1, p2)
    p3 = data_all[:, 14] - data_all[:, 8] # RShoulder - Thorax
    cos_gt_r = cos_func(p3, p2)
    valid_cos['Shoulder2Neck'] = torch.vstack((cos_gt_l, cos_gt_r))

    # Arm2ShoulderPlane
    p0 = data_all[:, 11] - data_all[:, 8] # LSHoulder - Thorax
    p1 = data_all[:, 14] - data_all[:, 8] # RSHoulder - Thorax
    n0 = torch.cross(p0, p1) # Shoulder plane
    p0 = data_all[:, 13] - data_all[:, 12] # LWrist - LElbow
    cos_gt_r = cos_func(p0, n0)
    p1 = data_all[:, 16] - data_all[:, 15] # RWrist - RElbow
    cos_gt_r = cos_func(p1, n0)
    valid_cos['Arm2ShoulderPlane'] = torch.vstack((cos_gt_l, cos_gt_r))

    # UpperSpine2LowerSpine
    p0 = data_all[:, 8] - data_all[:, 7] # Thorax - Spine
    p1 = data_all[:, 0] - data_all[:, 7] # Hip - Spine
    cos_gt = cos_func(p0, p1)
    valid_cos["UpperSpine2LowerSpine"] = cos_gt

    # Thigh2TorsoPlane
    p0 = data_all[:, 0] - data_all[:, 11] # Hip - LShoulder
    p1 = data_all[:, 0] - data_all[:, 14] # Hip - RShoulder
    n0 = torch.cross(p0, p1) # Torso plane

    p3 = data_all[:, 5] - data_all[:, 4] # LKnee - LHip
    p4 = data_all[:, 2] - data_all[:, 1] # RKnee - RHip

    cos_gt_l = cos_func(p3, n0)
    cos_gt_r = cos_func(p4, n0)
    valid_cos['Thigh2TorsoPlane'] = torch.vstack((cos_gt_l, cos_gt_r))

    # HeadPlane2TorsoPlane
    p0 = data_all[:, 10] - data_all[:, 11] # Head - LShoulder
    p1 = data_all[:, 10] - data_all[:, 14] # Head - RShoulder
    n1 = torch.cross(p0, p1) # Head plane
    valid_cos['HeadPlane2TorsoPlane'] = cos_func(n0, n1)
