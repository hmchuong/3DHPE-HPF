...
def parse_args():
    ...
    # Loss function
    parser.add_argument('--lambda-ang-loss', default=0., type=float, help='weight for AngLoss, if == 0, we skip this loss')
    parser.add_argument('--lambda-ang-limit', default=0., type=float, help='weight for AngLimit, if == 0, we skip this loss')
    parser.add_argument('--lambda-wao', default=0.5, type=float, help='weight for WAO, if == 0, we skip this loss')
    parser.add_argument('--lambda-ohjm', default=0.6, type=float, help='weight for mpjpe loss, if == 0, we skip this loss')
    parser.add_argument('--m-ohjm', default=0.7, type=float, help='m parameter for OHJM loss. if m=1, ohjm become mpjpe')
    parser.add_argument('--omega-wao', default=5e-3, type=float, help='omega for WAO loss. if omega=-1, WAO become AngOrient')
    parser.add_argument('--epsilon-wao', default=1e-3, type=float, help='epsilon for WAO loss. if epsilon=-1, WAO become AngOrient')
