...
if not args.evaluate:
    ...
    while epoch < args.epochs:
        ...
        for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():
            ...
            ang_loss_val = torch.tensor(0).to(predicted_3d_pos.device)
            ang_limit_val = torch.tensor(0).to(predicted_3d_pos.device)
            ang_orient_val = torch.tensor(0).to(predicted_3d_pos.device)
            mpjpe_val = torch.tensor(0).to(predicted_3d_pos.device)
            if args.lambda_ang_loss > 0:
                ang_loss_val = args.lambda_ang_loss * ang_loss(predicted_3d_pos, inputs_3d)
            if args.lambda_ang_limit > 0:
                ang_limit_val = args.lambda_ang_limit * ang_limit(predicted_3d_pos, inputs_3d)
            if args.lambda_wao > 0:
                ang_orient_val = args.lambda_wao * angle_orientation(predicted_3d_pos, args.omega_wao, args.epsilon_wao)
            if args.lambda_ohjm > 0:
                mpjpe_val = mpjpe(predicted_3d_pos, inputs_3d, top_k=args.m_ohjm)

            

            loss_total = args.lambda_ang_loss * ang_loss_val + \
                        args.lambda_ang_limit * ang_limit_val + \
                        args.lambda_wao * ang_orient_val + \
                        args.lambda_ohjm * mpjpe_val

            epoch_loss_3d_train += inputs_3d.shape[0] * inputs_3d.shape[1] * loss_total.item()
            N += inputs_3d.shape[0] * inputs_3d.shape[1]

            if batch_idx % 100 == 0:
                print("Training: Epoch {} - Batch {}/{} - AngLoss: {:.4f} - AngLimit: {:.4f} - WAO: {:.4f} - OHJM: {:.4f} - total: {:.4f} - avg. loss: {:.4f}".format(
                    epoch + 1, batch_idx + 1, train_generator.num_batches, ang_loss_val.item(), ang_limit_val.item(), ang_orient_val, mpjpe_val, loss_total.item(), epoch_loss_3d_train / N))
